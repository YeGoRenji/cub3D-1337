...
/*   Updated: 2024/02/14 18:08:36 by ylyoussf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_configs(t_vars *vars)
{
	...
	vars->nletter_tex = load_tex_png(N_TEX);
	vars->sky_tex = load_tex_png(SKYBOX_TEX);
	vars->light_status = false;
	vars->light_on = load_tex_png(FLASHON_TEX);
	vars->light_off = load_tex_png(FLASHOFF_TEX);
